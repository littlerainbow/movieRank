...
/***/ function(module, exports, __webpack_require__) {
	...
	var $ = __webpack_require__(2),
	    setPosterTitle = __webpack_require__(5),
	    setPopUpInfo = __webpack_require__(6),
	    twoRandomItems = __webpack_require__(7),
	    startVote = __webpack_require__(8),
	    endVote = __webpack_require__(9),
	    ranking = __webpack_require__(10),
	    saveToLocalStorage = __webpack_require__(12),
	    displayResults = __webpack_require__(13);

	var top10 = __webpack_require__(11);

	$(document).ready(function () {

	    var ItemsForCompare = twoRandomItems(top10),
	    ...
	    $('.start').on('click', function () {
	        $('.main-page').animate({ top: '-200vh' }, 700, function () {
	            $(undefined).css('dispaly', 'none');
	            startVote();
	        });
	    });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var type=jQuery.type(obj);return (type==="number"||type==="string")&&obj-parseFloat(obj)+1>=0;},isPlainObject:function isPlainObject(obj){ // Not plain objects:
	...
	return true;},isEmptyObject:function isEmptyObject(obj){var name;for(name in obj){return false;}return true;},type:function type(obj){if(obj==null){return obj+"";} // Support: Android<4.0 (functionish RegExp)
	...
	makeArray:function makeArray(arr,results){var ret=results||[];if(arr!=null){if(isArrayLike(Object(arr))){jQuery.merge(ret,typeof arr==="string"?[arr]:arr);}else {push.call(ret,arr);}}return ret;},inArray:function inArray(elem,arr,i){return arr==null?-1:indexOf.call(arr,elem,i);}, // Support: Android<4.1, PhantomJS<2
	...
	fired:function fired(){return !!_fired;}};return self;};function Identity(v){return v;}function Thrower(ex){throw ex;}jQuery.extend({Deferred:function Deferred(func){var tuples=[ // action, add listener, callbacks,
	...
	deferred[tuple[1]](function(){var returned=fn&&fn.apply(this,arguments);if(returned&&jQuery.isFunction(returned.promise)){returned.promise().progress(newDefer.notify).done(newDefer.resolve).fail(newDefer.reject);}else {newDefer[tuple[0]+"With"](this===_promise?newDefer.promise():this,fn?[returned]:arguments);}});});fns=null;}).promise();},then:function then(onFulfilled,onRejected,onProgress){var maxDepth=0;function resolve(depth,deferred,handler,special){return function(){var that=this===_promise?undefined:this,args=arguments,mightThrow=function mightThrow(){var returned,then; // Support: Promises/A+ section 2.3.3.3.3
	...
	maxDepth++;then.call(returned,resolve(maxDepth,deferred,Identity,special),resolve(maxDepth,deferred,Thrower,special),resolve(maxDepth,deferred,Identity,deferred.notify));} // Handle all other returned values
	}else { // Only substitute handlers pass on context
	// and multiple values (non-spec behavior)
	if(handler!==Identity){that=undefined;args=[returned];} // Process the value(s)
	// Default process is resolve
	(special||deferred.resolveWith)(that||deferred.promise(),args);}}, // Only normal processors (resolve) catch and reject exceptions
	process=special?mightThrow:function(){try{mightThrow();}catch(e){if(jQuery.Deferred.exceptionHook){jQuery.Deferred.exceptionHook(e,process.stackTrace);} // Support: Promises/A+ section 2.3.3.3.4.1
	// https://promisesaplus.com/#point-61
	// Ignore post-resolution exceptions
	if(depth+1>=maxDepth){ // Only substitute handlers pass on context
	// and multiple values (non-spec behavior)
	if(handler!==Thrower){that=undefined;args=[e];}deferred.rejectWith(that||deferred.promise(),args);}}}; // Support: Promises/A+ section 2.3.3.3.1
	...
	deferred[tuple[0]]=function(){deferred[tuple[0]+"With"](this===deferred?_promise:this,arguments);return this;}; // deferred.notifyWith = list.fireWith
	...
	when:function when(){var method,resolveContexts,i=0,resolveValues=_slice.call(arguments),length=resolveValues.length, // the count of uncompleted subordinates
	remaining=length, // the master Deferred.
	master=jQuery.Deferred(), // Update function for both resolving subordinates
	updateFunc=function updateFunc(i){return function(value){resolveContexts[i]=this;resolveValues[i]=arguments.length>1?_slice.call(arguments):value;if(! --remaining){master.resolveWith(resolveContexts.length===1?resolveContexts[0]:resolveContexts,resolveValues);}};}; // Add listeners to promise-like subordinates; treat others as resolved
	if(length>0){resolveContexts=new Array(length);for(;i<length;i++){ // jQuery.Deferred - treated specially to get resolve-sync behavior
	if(resolveValues[i]&&jQuery.isFunction(method=resolveValues[i].promise)){method.call(resolveValues[i]).done(updateFunc(i)).fail(master.reject); // Other thenables
	}else if(resolveValues[i]&&jQuery.isFunction(method=resolveValues[i].then)){method.call(resolveValues[i],updateFunc(i),master.reject);}else {updateFunc(i)(resolveValues[i]);}} // If we're not waiting on anything, resolve the master
	}else {master.resolveWith();}return master.promise();}}); // These usually indicate a programmer mistake during development,
	// warn about them ASAP rather than swallowing them by default.
	var rerrorNames=/^(Eval|Internal|Range|Reference|Syntax|Type|URI)Error$/;jQuery.Deferred.exceptionHook=function(error,stack){ // Support: IE9
	// Console exists when dev tools are open, which can happen at any time
	if(window.console&&window.console.warn&&error&&rerrorNames.test(error.name)){window.console.warn("jQuery.Deferred exception: "+error.message,stack);}}; // The deferred used on DOM ready
	...
	elem=el||elem;return jQuery.css(elem,"display")==="none"||!jQuery.contains(elem.ownerDocument,elem);};var swap=function swap(elem,options,callback,args){var ret,name,old={}; // Remember the old values, and insert the new ones
	...
	if(display==="none"){values[index]=dataPriv.get(elem,"display")||null;if(!values[index]){elem.style.display="";}}if(elem.style.display===""&&jQuery.css(elem,"display")==="none"&& // Support: Firefox <=42 - 43
	// Don't set inline display on disconnected elements with computed display: none
	jQuery.contains(elem.ownerDocument,elem)){values[index]=getDefaultDisplay(elem);}}else {if(display!=="none"){values[index]="none"; // Remember what we're overwriting
	...
	if(scripts){j=0;while(elem=tmp[j++]){if(rscriptType.test(elem.type||"")){scripts.push(elem);}}}}return fragment;}(function(){var fragment=document.createDocumentFragment(),div=fragment.appendChild(document.createElement("div")),input=document.createElement("input"); // Support: Android 4.0-4.3
	// Check state lost if the name is set (#11217)
	// Support: Windows Web Apps (WWA)
	// `name` and `type` must use .setAttribute for WWA (#14901)
	input.setAttribute("type","radio");input.setAttribute("checked","checked");input.setAttribute("name","t");div.appendChild(input); // Support: Android<4.2
	...
	fn=data;data=selector;selector=undefined;}}if(fn===false){fn=returnFalse;}if(one===1){origFn=fn;fn=function fn(event){ // Can use an empty set, since event contains the info
	...
	var i,prop,copy,type=event.type,originalEvent=event,fixHook=this.fixHooks[type];if(!fixHook){this.fixHooks[type]=fixHook=rmouseEvent.test(type)?this.mouseHooks:rkeyEvent.test(type)?this.keyHooks:{};}copy=fixHook.props?this.props.concat(fixHook.props):this.props;event=new jQuery.Event(originalEvent);i=copy.length;while(i--){prop=copy[i];event[prop]=originalEvent[prop];} // Support: Safari 6-8+
	...
	},ignored);}});jQuery.each({appendTo:"append",prependTo:"prepend",insertBefore:"before",insertAfter:"after",replaceAll:"replaceWith"},function(name,original){jQuery.fn[name]=function(selector){var elems,ret=[],insert=jQuery(selector),last=insert.length-1,i=0;for(;i<=last;i++){elems=i===last?this:this.clone(true);jQuery(insert[i])[original](elems); // Support: Android<4.1, PhantomJS<2
	// .get() because push.apply(_, arraylike) throws on ancient WebKit
	push.apply(ret,elems.get());}return this.pushStack(ret);};});var rmargin=/^margin/;var rnumnonpx=new RegExp("^("+pnum+")(?!px)[a-z%]+$","i");var getStyles=function getStyles(elem){ // Support: IE<=11+, Firefox<=30+ (#15098, #14150)
	...
	function computeStyleTests(){div.style.cssText="box-sizing:border-box;"+"position:relative;display:block;"+"margin:auto;border:1px;padding:1px;"+"top:1%;width:50%";div.innerHTML="";documentElement.appendChild(container);var divStyle=window.getComputedStyle(div);pixelPositionVal=divStyle.top!=="1%";reliableMarginLeftVal=divStyle.marginLeft==="2px";boxSizingReliableVal=divStyle.width==="4px"; // Support: Android 4.0 - 4.3 only
	...
	if(boxSizingReliableVal==null){computeStyleTests();}return reliableMarginLeftVal;}});})();function curCSS(elem,name,computed){var width,minWidth,maxWidth,ret,style=elem.style;computed=computed||getStyles(elem); // Support: IE9
	...
	if(elem.getClientRects().length){val=elem.getBoundingClientRect()[name];} // Support: IE11 only
	// In IE 11 fullscreen elements inside of an iframe have
	// 100x too small dimensions (gh-1764).
	if(document.msFullscreenElement&&window.top!==window){val*=100;} // Some non-html elements return undefined for offsetWidth, so check for null/undefined
	...
	return tween;}}}function defaultPrefilter(elem,props,opts){ /* jshint validthis: true */var prop,value,toggle,hooks,oldfire,propTween,restoreDisplay,display,isBox="width" in props||"height" in props,anim=this,orig={},style=elem.style,hidden=elem.nodeType&&isHidden(elem),dataShow=dataPriv.get(elem,"fxshow"); // Queue-skipping animations hijack the fx hooks
	...
	if(isBox&&elem.nodeType===1){ // Support: IE 9 - 11
	...
	propTween=createTween(hidden?dataShow[prop]:0,prop,anim);if(!(prop in dataShow)){dataShow[prop]=propTween.start;if(hidden){propTween.end=propTween.start;propTween.start=prop==="width"||prop==="height"?1:0;}}}}function propFilter(props,specialEasing){var index,name,easing,value,hooks; // camelCase, specialEasing and expand cssHook pass
	...
	jQuery.fn.delay=function(time,type){time=jQuery.fx?jQuery.fx.speeds[time]||time:time;type=type||"fx";return this.queue(type,function(next,hooks){var timeout=window.setTimeout(next,time);hooks.stop=function(){window.clearTimeout(timeout);};});};(function(){var input=document.createElement("input"),select=document.createElement("select"),opt=select.appendChild(document.createElement("option"));input.type="checkbox"; // Support: Android<4.4
	// Default value for a checkbox should be "on"
	support.checkOn=input.value!==""; // Support: IE<=11+
	// Must access selectedIndex to make default options select
	support.optSelected=opt.selected; // Support: IE<=11+
	// An input loses its value after becoming a radio
	input=document.createElement("input");input.value="t";input.type="radio";support.radioValue=input.value==="t";})();var boolHook,attrHandle=jQuery.expr.attrHandle, // Exclusively lowercase A-Z in attribute names (gh-2730)
	// https://dom.spec.whatwg.org/#converted-to-ascii-lowercase
	raz=/[A-Z]+/g,lowercase=function lowercase(ch){return ch.toLowerCase();};jQuery.fn.extend({attr:function attr(name,value){return access(this,jQuery.attr,name,value,arguments.length>1);},removeAttr:function removeAttr(name){return this.each(function(){jQuery.removeAttr(this,name);});}});jQuery.extend({attr:function attr(elem,name,value){var ret,hooks,nType=elem.nodeType; // Don't get/set attributes on text, comment and attribute nodes
	...
	return jQuery.trim(elem.value);}},select:{get:function get(elem){var value,option,options=elem.options,index=elem.selectedIndex,one=elem.type==="select-one",values=one?null:[],max=one?index+1:options.length,i=index<0?max:one?index:0; // Loop through all the selected options
	...
	var handler=function handler(event){jQuery.event.simulate(fix,event.target,jQuery.event.fix(event));};jQuery.event.special[fix]={setup:function setup(){var doc=this.ownerDocument||this,attaches=dataPriv.access(doc,fix);if(!attaches){doc.addEventListener(orig,handler,true);}dataPriv.access(doc,fix,(attaches||0)+1);},teardown:function teardown(){var doc=this.ownerDocument||this,attaches=dataPriv.access(doc,fix)-1;if(!attaches){doc.removeEventListener(orig,handler,true);dataPriv.remove(doc,fix);}else {dataPriv.access(doc,fix,attaches);}}};});}var location=window.location;var nonce=jQuery.now();var rquery=/\?/;jQuery.parseJSON=JSON.parse; // Cross-browser xml parsing
	jQuery.parseXML=function(data){var xml;if(!data||typeof data!=="string"){return null;} // Support: IE9
	try{xml=new window.DOMParser().parseFromString(data,"text/xml");}catch(e){xml=undefined;}if(!xml||xml.getElementsByTagName("parsererror").length){jQuery.error("Invalid XML: "+data);}return xml;};var r20=/%20/g,rhash=/#.*$/,rts=/([?&])_=[^&]*/,rheaders=/^(.*?):[ \t]*([^\r\n]*)$/mg, // #7653, #8125, #8152: local protocol detection
	...
	jQuery.parseHTML=function(data,context,keepScripts){if(typeof data!=="string"){return [];}if(typeof context==="boolean"){keepScripts=context;context=false;} // Stop scripts or inline event handlers from being executed immediately
	...
	if(arguments.length){return options===undefined?this:this.each(function(i){jQuery.offset.setOffset(this,options,i);});}var docElem,win,rect,doc,elem=this[0];if(!elem){return;} // Support: IE<=11+
	...
	return arguments.length===1?this.off(selector,"**"):this.off(types,selector||"**",fn);}}); // Register as a named AMD module, since jQuery can be concatenated with other
...
/***/ function(module, exports, __webpack_require__) {
	...
	function setPosterTitle(movieSection, obj) {
	    var movieTitle = $(movieSection + ' h2'),
...
/***/ function(module, exports, __webpack_require__) {
	...
	var firstAwards = $('.first-film .awards-wrap'),
...
/***/ function(module, exports, __webpack_require__) {
	...
	function endVote() {
	    ...
	    firstMovieInfo.add(secondMovieInfo).add(posters).add(v).add(s).add(titles).add(votes).removeAttr('style');
	}
...
/***/ function(module, exports) {
	...
	function ranking(r1, r2, vote) {
	    ...
	    var expectedScoreR1 = 1 / (1 + Math.pow(10, (r2.userRating - r1.userRating) / 400)),
...
/***/ function(module, exports) {
	...
	var top10 = [{
	    ...
	    awards: ['American Society of Cinematographers', 'Awards of the Japanese Academy', 'Camerimage', 'Chlotrudis Awards', 'Dallas-Fort Worth Film Critics Association Awards', 'Directors Guild of America', 'Grammy Awards', 'Heartland Film Festival', 'Humanitas Prize', 'Kinema Junpo Awards', 'etc.'],
	    ...
	    worldRaiting: 1,
	    userRating: 400,
	    poster: 'https://geeknama.files.wordpress.com/2015/01/shawshank-redemption-poster.jpg'
	}, {
	    title: 'The Godfather',
	    actors: ['Marlon Brando', ' Al Pacino', ' James Caan'],
	    awards: ['Academy Awards', 'Golden Globes', 'BAFTA Awards', 'David di Donatello Awards', 'Directors Guild of America', 'Golden Schmoes Awards', 'Grammy Awards', 'Golden Screen, Germany', 'National Board of Review', 'National Film Preservation Board', 'etc.'],
	    storyLine: 'When the aging head of a famous crime family decides to transfer his position to one of his subalterns, a series of unfortunate events start happening to the family, and a war begins between all the well-known families leading to insolence, deportation, murder and revenge, and ends with the favorable successor being finally chosen.',
	    worldRaiting: 2,
	    userRating: 400,
	    poster: 'http://media-cache-ec0.pinimg.com/originals/fe/1a/26/fe1a265a96793baacd6d4c32a921905f.jpg'
	}, {
	    title: 'The Godfather: Part II',
	    actors: ['Al Pacino', ' Robert De Niro', 'Robert Duvall'],
	    awards: ['Academy Awards', 'BAFTA Awards', 'Directors Guild of America', 'Golden Schmoes Awards', 'National Society of Film Critics Awards', 'Kansas City Film Critics Circle Awards', 'National Board of Review', 'National Film Preservation Board', 'etc.'],
	    ...
	    worldRaiting: 3,
	    userRating: 400,
	    poster: 'https://privatehd.to/images/posters/q/d/6/qd6kul77pgzs.jpg'
	}, {
	    title: 'The Dark Knight',
	    actors: [' Christian Bale', ' Heath Ledger', 'Aaron Eckhart'],
	    awards: ['Academy Awards', 'BAFTA Awards', 'Golden Globes, USA', 'Screen Actors Guild Awards', 'Academy of Science Fiction', 'African-American Film Critics Association', 'Alliance of Women Film Journalists', 'Art Directors Guild', 'ASCAP Film and Television Music Awards', 'etc.'],
	    ...
	    worldRaiting: 4,
	    userRating: 400,
	    poster: 'http://picsmobi.net/uploads/pictures/others-iphone/104483-batman.jpg'
	}, {
	    title: 'Schindler\'s List',
	    actors: ['Liam Neeson', 'Ralph Fiennes', 'Ben Kingsley'],
	    awards: ['Academy Awards', 'BAFTA Awards', 'Golden Globes, USA', 'Amanda Awards, Norway', 'American Cinema Editors', 'Awards of the Japanese Academy', 'BMI Film & TV Awards', 'Boston Society of Film Critics Awards', 'British Society of Cinematographers', 'etc.'],
	    ...
	    worldRaiting: 5,
	    userRating: 400,
	    poster: 'http://www.hdclub.ua/files/film/big/bigi511cefdd947f7.jpg'
	}, {
	    title: '12 Angry Men',
	    actors: ['Henry Fonda', 'Lee J. Cobb', 'Martin Balsam'],
	    awards: ['Academy Awards', 'BAFTA Awards', 'Golden Globes, USA', 'Berlin International Film Festival', 'Blue Ribbon Awards', 'Bodil Awards', 'Directors Guild of America', 'Edgar Allan Poe Awards', 'Grand Prix de l\'UCC', 'etc.'],
	    ...
	    worldRaiting: 6,
	    userRating: 400,
	    poster: 'http://img.goldposter.com/2015/04/12-Angry-Men_poster_goldposter_com_16-508x715.jpg'
	}, {
	    title: 'Pulp Fiction',
	    actors: [' John Travolta', 'Uma Thurman', 'Samuel L. Jackson'],
	    awards: ['Academy Awards', 'BAFTA Awards', 'Golden Globes, USA', 'Screen Actors Guild Awards', 'Academy of Science Fiction', 'American Cinema Editors', 'American Comedy Awards', 'Australian Film Institute', 'Awards of the Japanese Academy', 'etc.'],
	    ...
	    worldRaiting: 7,
	    userRating: 400,
	    poster: 'http://st-im.kinopoisk.ru/im/poster/2/5/6/kinopoisk.ru-Pulp-Fiction-2561659.jpg'
	}, {
	    title: 'The Lord of the Rings: The Return of the King',
	    actors: ['Elijah Wood', 'Viggo Mortensen', ' Ian McKellen'],
	    awards: ['Academy Awards', 'BAFTA Awards', 'Golden Globes, USA', 'Screen Actors Guild Awards', 'Academy of Science Fiction', 'AFI Awards', 'Amanda Awards', 'American Cinema Editors', 'American Society of Cinematographers', 'etc.'],
	    ...
	    worldRaiting: 8,
	    userRating: 400,
	    poster: 'http://4.bp.blogspot.com/--SXqGeT0lIo/UsBngrfSS1I/AAAAAAAABpA/QQEhcfOqBQo/s1600/bb.jpg'
	}, {
	    title: 'Il buono, il brutto, il cattivo',
	    actors: ['Clint Eastwood', 'Eli Wallach', 'Lee Van Cleef'],
	    awards: ['Laurel Awards', 'Online Film & Television Association'],
	    storyLine: 'Three different men of three different tempers and tastes get involved in a long and full-of-adventure battle in order to find a fortune in gold. While the first man who is an ex-bounty hunter and a forgiving person knows the name of the cemetery which the gold is buried in, the second who is a fast-tempered greedy man knows the name on the grave. But the third person, a cruel cold-blooded murderer, knows none; so he has to reach the gold in his own way of finding something.',
	    worldRaiting: 9,
	    userRating: 400,
	    poster: 'http://cdn.kinolabel.ru/posters/full/39/c0/b5/65/3950cac341.jpg'
	}, {
	    title: 'Fight Club',
	    actors: [' Brad Pitt', 'Edward Norton', 'Meat Loaf'],
	    awards: ['Academy Awards', 'Awards Circuit Community Awards', 'Brit Awards', 'Costume Designers Guild Awards', 'Dallas-Fort Worth Film Critics Association Awards', 'DVD Exclusive Awards', 'Empire Awards', 'Jupiter Award', 'MTV Movie Awards', 'etc.'],
	    ...
	    worldRaiting: 10,
	    userRating: 400,
	    poster: 'http://st.kp.yandex.net/im/poster/2/1/5/kinopoisk.ru-Fight-Club-2152279.jpg'
	}];
